...
void SalinitySampler::sample(){
	for ( int i = 0; i < SL_NUM_SAMPLES; i++){
		update();
	}
	save();
	cleanup();
...
void SalinitySampler::cleanup(){
	for ( int i = 0; i < SL_NUM_BURST_PINS+1; i++){
...
void SalinitySampler::update(){
	timestamp();
	for ( int i = 0; i < SL_NUM_BURST_PINS; i++){
		node* curr = new node;
...
void SalinitySampler::timestamp(){
