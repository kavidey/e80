...
filename= .001;
map = imread("phakelake.png");
map = flip(map);
imgdim = size(map);
xdim = imgdim(1);
ydim = imgdim(2);
realx = 21 % real map width, in meters, approximated using google maps
realy = 17.5 % real map height, in meters, approximated using google maps
...
xScaleFactor = 1;
xShift = 10.1; % origin is ~60m east of plot origin
yScaleFactor = 1;
yShift = 13.3; % origin is ~110m north of plot origin
